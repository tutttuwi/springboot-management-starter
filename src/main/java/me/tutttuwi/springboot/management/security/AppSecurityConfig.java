...
public class AppSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  public void configure(HttpSecurity http) throws Exception {
    ...
    http.authorizeRequests().antMatchers(STATIC_PERMIT_PATH).permitAll()
        ...
        .logoutSuccessUrl("/login").permitAll() // すべてのユーザーに対してログインフォームへのアクセス件を付与する
    ...
    // WebSecurityConfigurerAdapterを継承して、コンフィギュレーションクラスを作成している場合は、
    // sessionManagementメソッドは親クラスの処理で呼び出されるため、デフォルトでセッション管理機能が適用されている
