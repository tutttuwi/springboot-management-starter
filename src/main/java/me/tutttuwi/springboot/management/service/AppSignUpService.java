...
public class AppSignUpService {
   ...
   * アカウント情報登録.
   *
   * @param form SignUpUserFormRequest
   */
  @Transactional
  public void registUserInfo(SignUpUserFormRequest form) throws Throwable {
    ...
    numbering =
        numberingDao.selectByNumKey(confReader.getNumKey(), SelectOptions.get().forUpdate());
    ...
    BoundMapperFacade<SignUpUserFormRequest, AccountInfo> boundMapperFacadeInfo =
        mapperFactory.getMapperFacade(SignUpUserFormRequest.class, AccountInfo.class);
    ...
    authKey.setExpireDt(
        Timestamp.valueOf(LocalDateTime.now().plusHours(confReader.getAuthEmailValidhour())));
  ...
  private void sendAuthEmail(AccountIndiv accountIndiv, String emailAuthKey) {
    ...
    String emailAuthUrl = UriComponentsBuilder.fromUriString(confReader.getAuthEmailUrl())
        .queryParam("key", emailAuthKey).toUriString();
    ...
    mailer.send(mailer -> mailer.from(msg.getMsg("mail.signup.from"))
        .to(msg.getMsg("mail.signup.to")).subject(msg.getMsg("mail.signup.subject"))
