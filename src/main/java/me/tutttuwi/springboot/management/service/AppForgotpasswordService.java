...
public class AppForgotpasswordService {
  ...
  public void sendPasswordUpdateEmail(ForgotpasswordFormRequest inputForm) throws Throwable {
    ...
    authKey.setExpireDt(
        Timestamp.valueOf(LocalDateTime.now().plusHours(confReader.getPasswordEmailValidhour())));
    ...
    mailer.send(mailer -> mailer.from(MessageUtils.getMsg("mail.forgotpassword.from"))
        .to(MessageUtils.getMsg("mail.forgotpassword.to"))
        .subject(MessageUtils.getMsg("mail.forgotpassword.subject"))
        .body(mailTempl.getText(MessageUtils.getMsg("mail.forgotpassword.body.path"), context)));
